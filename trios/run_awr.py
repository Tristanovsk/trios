...
for idpr in idprs:
    ...
    if awr.Edf:
        ...
        # data processing
        # -----------------------------
        Rrs, rho = awr.process_wrapper(wl, raw, raw.sza, ws=ws, azi=azi)
        Rrs_15, rho = awr.process_wrapper(wl, raw, raw.sza, ws=ws, azi=azi, method='M15')
        Rrs_h, rho = awr.process_wrapper(wl, raw, raw.sza, ws=ws, azi=azi, method='osoaa')
        ...
        add_curve(ax, wl, Rrs_h[ind].transpose().mean(axis=1), Rrs_h[ind].transpose().std(axis=1), label='H, QC-passed',
                  c='green')
        add_curve(ax, wl, Rrs_15[ind].transpose().mean(axis=1), Rrs_15[ind].transpose().std(axis=1),
                  label='M15, QC-passed',
                  c='violet')
        ...
        fig.savefig(os.path.join(dirfig, 'trios_awr_' + date + '_' + name + '_idpr' + idpr + '.png'))
        plt.close()
