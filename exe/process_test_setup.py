...
for azi, Lt1 in Lt0.groupby(level=2):
    for vza,Lt in Lt1.groupby(level=1):
        ...
        rho15 = awr.get_rho_mobley(awr.rhoM2015,[df.sza.min()],[vza],[azi],[ws],method='M15')
        rho99 = awr.get_rho_mobley(awr.rhoM1999,[df.sza.min()],[vza],[azi],[ws],method='M99')

        Rrs_h =(df.loc[:,'Lt'] -rho_h*df.loc[:,'Lsky'])/ df.loc[:,'Ed']
        Rrs15 = (df.loc[:,'Lt'] -float(rho15)*df.loc[:,'Lsky'])/ df.loc[:,'Ed']

        Rrs99 = (df.loc[:,'Lt'] -float(rho99)*df.loc[:,'Lsky'])/ df.loc[:,'Ed']
        #plt.figure()
